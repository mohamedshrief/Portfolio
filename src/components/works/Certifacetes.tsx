...
import { certificatesData } from "@/Data/CertifacatesData";
import CertificateCard from "./CertificateCard";

export default function Certifacetes() {
  ...
  const allVisible = visibleCount >= certificatesData.length;
  // const minVisible = visibleCount <= 3;

  return (
    <>
      <CardsLayout>
        {certificatesData.slice(0, visibleCount).map((certificate) => (
      ...
      <div className="w-full flex justify-center gap-4">
        ...
        >
          {allVisible ? "No More Certificates" : "See More"}
        </button>
        {/* <button
